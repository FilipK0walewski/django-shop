...
def main():
    conn = psycopg2.connect(host='localhost', dbname='hagne', user='filip', password='123')
    cur = conn.cursor()
    ...
    for i in all_categories.keys():
        for category_name, parent in all_categories[i]:
            ...
            cur.execute('select id from shop_category where name = %s', (category_name,))
            category_id = cur.fetchone()
            if not category_id:
                cur.execute('insert into shop_category(name, parent_id) values (%s, %s) returning id', (category_name, parent_id))
                category_id = cur.fetchone()
    ...
    upsert_query = '''
        insert into shop_product(manufacturer, product_id, ean, quantity, description, name, price, tax_rate, color, category_id)
    ...
    for product_id, product_images in images.items():
        for image in product_images:
            ...
            cur.execute('insert into shop_image(url, product_id) values (%s, %s) on conflict (url) do nothing', (image, db_product_id))
